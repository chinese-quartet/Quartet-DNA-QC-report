...
dnaseqc <- function(variant_qc_file,
  ...
  ## 转换成DataFrame
  mendelian_df <- data.frame(matrix(one_set, ncol = 5, byrow = TRUE), stringsAsFactors =
                               FALSE)
  # 设置列名
  names(mendelian_df) <- c('sample',
                           'group',
                           'batch',
                           'snv_mendelian',
                           'indel_mendelian')
  ...
  quality_metrics_df <- do.call(rbind,
